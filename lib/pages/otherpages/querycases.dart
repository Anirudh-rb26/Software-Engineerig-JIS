...
class QueryCases extends StatefulWidget {
  QueryCases({super.key});

  @override
  State<QueryCases> createState() => _QueryCasesState();
}

class _QueryCasesState extends State<QueryCases> {
  ...
  void searchForCaseId(String query, List<String> caseIds) {
    if (caseIds.contains(query)) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => CasePage(documentId: query),
        ),
      );
    } else {
